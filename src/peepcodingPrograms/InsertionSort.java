...
public class InsertionSort {
	public static void main(String args[])
	{
		...
		for(int i=1; i<arr.length; i++)
		{
			...
			while(j>=0 && arr[j] > current_value)// while j value is greater and equal to index value 0 and also arr of j greater than current value // here j is not greater than 9 so it will not enter to while
			//2 yes 9 is greater than 1
			{
				arr[j+1] = arr[j]; // so at array 2 9 is placed
				j--; // j is again at position 1 
			}
			arr[j+1] = current_value; // array j + 1 is index 2 where already there is cureent value 
			// again 
		}
	}
